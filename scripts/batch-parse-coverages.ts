...
function parseMdFile(content: string): CoverageRecord[] {
  ...
  for (const line of lines) {
    ...
    const [serialNumber, policyDocumentId, coverageType, coverageName, clauseText] = cleanedParts;
...
async function main() {
  ...
  const endNum = parseInt(process.argv[3] || '150');
  ...
  console.log(`✅ 找到 ${records.length} 条记录（序号 ${startNum}-${endNum}，共${allRecords.length}条）`);
