 ...
 *   ts-node ../../scripts/import-all-batches.ts [起始批次] [结束批次]
 * 
 * 示例:
 *   ts-node ../../scripts/import-all-batches.ts 1 15  # 导入批次1到15（约3000条）
 *   ts-node ../../scripts/import-all-batches.ts 1 5   # 只导入批次1到5（约1000条）
 */
...
const backendDir = path.resolve(scriptsDir, '../coverage-parser/backend');
const 解析结果目录 = path.resolve(scriptsDir, '../解析结果');

// 动态导入（需要从backend目录导入）
const { coverageLibraryStorage } = require(path.join(backendDir, 'src/services/parser/storage/coverageLibraryStorage'));
